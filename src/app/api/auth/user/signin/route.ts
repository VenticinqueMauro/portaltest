...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (userFound) {
            return NextResponse.json({ message: 'Este correo electrónico ya ha sido registrado' }, { status: 400 }) // Devuelve un mensaje indicando que el correo ya está registrado si se encuentra en la base de datos
        }
        ...
        await sendEmail({
            from: 'Portal de Prueba <onboarding@resend.dev>',
            to: [email],
        ...
        return NextResponse.json({ message: 'Registro exitoso' }, { status: 200 }) // Devuelve un mensaje de registro exitoso si todo fue exitoso
